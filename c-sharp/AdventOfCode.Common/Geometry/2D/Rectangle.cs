	...
	where T : unmanaged, IBinaryInteger<T>
{
	...
	public IEnumerable<Point2D<T>> CornerPoints
	{
		get
		{
			yield return new Point2D<T>(XRange.Start, YRange.Start);
			yield return new Point2D<T>(XRange.Start, YRange.End);
			yield return new Point2D<T>(XRange.End, YRange.Start);
			yield return new Point2D<T>(XRange.End, YRange.End);
		}
	}
