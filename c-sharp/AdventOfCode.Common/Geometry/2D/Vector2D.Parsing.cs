	...
	where T : unmanaged, INumber<T>
{
	...
	public static bool TryParse([NotNullWhen(true)] string? s, IFormatProvider? provider,
		[MaybeNullWhen(false)] out Vector2D<T> result)
	{
		ArgumentNullException.ThrowIfNull(s);
	...
	///     the string representations of the <see cref="X" /> and <see cref="Y" /> values.
	/// </remarks>
	public static bool TryParse([NotNullWhen(true)] ReadOnlySpan<char> s, IFormatProvider? provider,
		[MaybeNullWhen(false)] out Vector2D<T> result)
	{
		result = default;
		Span<T> values = stackalloc T[_parsingValueCount];
		...
		if (parsed) result = new Vector2D<T>(values[0], values[1]);
		return parsed;
	}
}
