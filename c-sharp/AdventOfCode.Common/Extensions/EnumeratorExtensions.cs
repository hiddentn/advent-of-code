...
public static class EnumeratorExtensions
{
	...
	/// <exception cref="ArgumentNullException">Thrown when the <paramref name="enumerator" /> is <see langword="null" />.</exception>
	/// <exception cref="InvalidOperationException">Throws when the end of the collection has been reached.</exception>
	public static void EnsureMoveToNextNonEmptyLine(this IEnumerator<string> enumerator, string? message = null,
		bool skipCurrent = true)
	{
		ArgumentNullException.ThrowIfNull(enumerator);
		if (skipCurrent) enumerator.EnsureMoveNext(message);
		while (string.IsNullOrWhiteSpace(enumerator.Current)) enumerator.EnsureMoveNext(message);
	}
}
