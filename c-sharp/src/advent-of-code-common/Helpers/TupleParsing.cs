...
internal static class TupleParsing
{
	...
	///     and write resulting values into the specified <see langword="in" /> <see cref="Span{T}" />
	...
	/// </remarks>
	public static void ParseValueTupleIntoSpan<T>(ReadOnlySpan<char> s, IFormatProvider? provider, in Span<T> result,
		TupleParsingOptions options)
		where T : unmanaged, ISpanParsable<T>
	{
		...
		{
			var trimmedS = s.Trim();
			if (trimmedS.IsEmpty) throw new FormatException("The input text is empty.");
			...
			foreach (var part in splitter)
			{
				if (partCount >= result.Length)
					throw new FormatException(
						$"Too many values. Expected {result.Length}, found at least {partCount}.");
	...
	///     to modify the parsing behavior.
	/// </remarks>
	public static bool TryParseValueListIntoSpan<T>(ReadOnlySpan<char> s, IFormatProvider? provider, in Span<T> result,
		TupleParsingOptions options)
		where T : unmanaged, ISpanParsable<T>
	{
		if (options.Count != result.Length)
		...
		if (options.HasBrackets)
		{
			if (s[0] == options.OpeningBracket && s[^1] == options.ClosingBracket)
				s = s[1..^1];
			else if (options.BracketsRequired) return false;
		}
		else if (options.BracketsRequired)
		{
			return false;
		...
		foreach (var part in splitter)
		{
			if (partCount >= result.Length) return false;
			result[partCount++] = T.Parse(part, provider);
		}
	...
	public readonly struct TupleParsingOptions
	{
		...
		/// </summary>
		public TupleParsingOptions(int count, char separator, char openingBracket, char closingBracket,
